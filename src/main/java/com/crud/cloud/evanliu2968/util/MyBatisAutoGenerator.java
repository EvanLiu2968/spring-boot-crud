...
public class MyBatisAutoGenerator {
     ...
     */
    private static final String DB_URL = "jdbc:mysql://192.168.2.143:3306/spring-boot-crud?useSSL=false&useUnicode=true&characterEncoding=UTF-8" +
            "&allowMultiQueries=true&serverTimezone=Asia/Shanghai&useInformationSchema=true";
    private static final String DB_USER = "root";
    private static final String DB_PASSWORD = "root";

    private static final String PATH_DO = System.getProperty("user.dir").replace('\\', '/') + "/src/main/java/com/crud/cloud/evanliu2968/po";
    private static final String PATH_DAO = System.getProperty("user.dir").replace('\\', '/') + "/src/main/java/com/crud/cloud/evanliu2968/dao";
    private static final String PATH_SERVICE = System.getProperty("user.dir").replace('\\', '/') + "/src/main/java/com/crud/cloud/evanliu2968/service";
    private static final String PATH_SERVICE_IMP = System.getProperty("user.dir").replace('\\', '/') + "/src/main/java/com/crud/cloud/evanliu2968/service/impl";
    private static final String PATH_MAPPER = System.getProperty("user.dir").replace('\\', '/') + "/src/main/java/com/crud/cloud/evanliu2968/dao/xml";

    private static final String PACKAGE_NAME_DAO = "com.crud.cloud.evanliu2968.dao";
    private static final String PACKAGE_NAME_DO = "com.crud.cloud.evanliu2968.po";
    private static final String PACKAGE_NAME_SERVICE = "com.crud.cloud.evanliu2968.service";

    private static final String KEY_COLUMN_NAME = "COLUMN_NAME";
    private static final String KEY_TYPE_NAME = "TYPE_NAME";
    private static final String KEY_REMARKS = "REMARKS";

    private static final String AUTHOR = "guohao.yang";
...
//        generator.generate("%");
        ...
        //盘点+继任
//        generator.generate("inventory_job");
//        generator.generate("inventory_job_config");
//        generator.generate("inventory_job_person");
//        generator.generate("inventory_job_schedule");
//        generator.generate("inventory_schedule_person");
//        generator.generate("inventory_job_result");
//        generator.generate("inventory_result_risk");
//
//        generator.generate("post_person_ref");

//        generator.generate("inventory_result_succession");

//        generator.generate("inventory_person_tracker");
...
//                System.out.println(columnName + " " + columnType + " " + remarks);
            ...
            if (null != tableInfo && tableInfo.size() > 0) {
                this.generateEntity(setPascalCase(tableName) + "PO", tableInfo, tableName, tableRemark);

                this.generateMapper(tableName, tableInfo);

                this.generateDAO(setPascalCase(tableName), tableRemark);

                //生成初始化的service接口
                this.generateService(setPascalCase(tableName), tableRemark);

                //生成初始化的service实现
                this.generateServiceImp(setPascalCase(tableName), tableRemark);
...
//                this.generateAlterIds(tableName, (String) tableInfo.get(0).get(KEY_REMARKS));
    ...
    private void generateAlterIds(String tableName, String pkRemark) {
        ...
        buf.append("ALTER TABLE `crud`.`" + tableName + "`\n");
...
//            FileUtil.mkDirs(path_do);
            ...
            buf.append("import com.baomidou.mybatisplus.annotations.TableLogic;\n");
            ...
            while (var12.hasNext()) {
                ...
                if (i == 0) {
                    ...
                    if (fieldCamel.equals("version")) {
                        //@TableField(value="version", update="%s+1")
                        buf.append("    @TableField(value=\"version\", update=\"%s+1\")\n");
                    } else {
                        buf.append("    @TableField(\"" + field + "\")\n");
                    }
                }
                i++;
                if (fieldCamel.equals("delFlag")) {
                    //@TableLogic
                    buf.append("    @TableLogic\n");
                }
...
//            FileUtils.forceDelete(new File(fileName));
    ...
    private void generateDAO(String className, String remark) throws Exception {
        ...
        buf.append("import " + PACKAGE_NAME_DO + "." + className + "PO;\n");
        ...
        //3、类定义
        buf.append("public interface ").append(className).append("Mapper extends BaseMapper<" + className + "PO> {\n\n");
        buf.append("}");
    ...
    private void generateService(String className, String remark) throws Exception {
        ...
        //3、类定义
        buf.append("public interface ").append(className).append("Service extends IService<" + className + "PO> {\n\n");
        buf.append("}");
    ...
    private void generateServiceImp(String className, String remark) throws Exception {
        ...
        //3、类定义
        buf.append("@Primary\n").append("@Service\n");
        buf.append("public class ").append(className).append("ServiceImpl extends ServiceImpl<" + className + "Mapper, " +
                className + "PO> implements " + className + "Service {\n\n");
        buf.append("}");
    ...
    private void generateMapper(String tableName, List<Map<String, Object>> tableInfo) throws Exception {
        if (tableInfo != null && tableInfo.size() > 0) {
            ...
            buf.append("\t<resultMap id=\"BaseResultMap\" type=\"").append(PACKAGE_NAME_DO).append(".").append(className + "PO").append("\">\n");
    ...
    private String getType(String str) {
        ...
        if (StringUtil.isEmpty(str)) {
            ...
            if (str.equals("timestamp") || str.equals("date")) {
                return "Date";
            }
